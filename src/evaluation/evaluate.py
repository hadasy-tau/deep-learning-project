"""Scoring for the personalization arm: transcribe, count errors, compare.

Baseline and tuned models go through the identical path -- that is the point,
and it is what makes a base-vs-tuned comparison mean anything.

Error COUNTS are stored, never rates, so speaker WER stays
total_errors / total_words however the rows are later grouped.

`stage1_wer` and `like_for_like` used to live here. They re-scored personal-test
against Stage 1's long-form VoxKnesset numbers, off the ct2 dump's
...
def score(refs, hyps):
    ...
    insertions the model learned protocol style, not the speaker.
    """
    rows = []
...
def paired_bootstrap(base, tuned, n_boot=2000, seed=0):
    ...
    This is the measurement a power analysis can only predict: how small a
    base-vs-tuned gap the personal-test set can actually resolve.
    """
