...
# ---- small helpers --------------------------------------------------------
def _cached(name, fn):
    """JSON cache beside the module.  A copy of the speaker-index builder's
    own `_cached`, not an import: the two builders are deliberately independent,
    so neither can be made unloadable by the other."""
...
def _get(url, dest, tries=6, token=None):
    ...
    The retry shape is the speaker index's protocol.py::_dl -- capped jittered backoff, 404
...
def _check_index(fetch=False):
    ...
    `python src/preprocessing/chunk_corpus/chunks.py --fetch` to pull it and check for real."""
